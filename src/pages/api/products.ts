...
// بيانات تجريبية للمنتجات على مستوى API للتأكد من عملها حتى عند حدوث خطأ اتصال
const MOCK_PRODUCTS = [
  {
    id: 1,
    name: "هاتف ذكي متطور",
...
export default async function handler(
  ...
  try {
    if (req.method === 'GET') {
      ...
      try {
        // محاولة استخدام وظيفة getProducts للحصول على المنتجات من Supabase
        const products = await getProducts(options);
        return res.status(200).json({ products });
      } catch (error) {
        console.log("استخدام البيانات المحلية بسبب خطأ اتصال Supabase:", error);
        
        // تطبيق الفلترة يدويًا في حالة الفشل
